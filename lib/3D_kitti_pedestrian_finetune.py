...
def load_image_set_index():
    ...
    image_set_file = '/home/jackqian/KITTI/val.txt'
...
def draw_lidar(lidar, is_grid=False, is_top_region=False, fig=None):
    ...
    mlab.points3d(
        ...
        colormap='gnuplot',  #'gnuplot', 'bone',  #'spectral',  #'copper',
        scale_factor=10,
        figure=fig)
    ...
    if 1:
        mlab.points3d(0, 0, 0, color=(0,0,1), mode='sphere', scale_factor=0.2)
...
for i in range(303,304):
    ...
    print "i:", i
    print "image_index:", image_index[i]
    ...
    ######## draw_detect_corners
    if bool_ifshow == 1:
        # boxes3D_corners = load_kitti_annotation(image_index[i])["boxes_corners"]
        # corners = boxes3D_corners[:, :24].reshape((-1, 3, 8)).transpose((0, 2, 1))
        # draw_gt_boxes3d(corners, color=(0, 1, 0), fig=fig)
        pass

    ######## draw 3D boxes_gt in the image
    if bool_ifshow == 1:
        # image_name = image_path_from_index(image_index[i])
        # image = cv2.imread(image_name)
        # calib = calib_at(image_index[i])
        # image_cnr = show_lidar_corners(image, GT_boxes3D_corners[:, :24], calib)
        # # cv2.imshow("image_corners", image_cnr)
        # # cv2.waitKey(10)
        pass
    ...
    if bool_ifshow == 1:
        ...
        # image = cv2.imread(image_name)
        # calib = calib_at(image_index[i])
        # image_cnr = show_lidar_corners(image, boxes3D_corners[:, :24], calib)
        pass


    if bool_ifshow == 1:
        #cv2.imshow("image", image_cnr)
        mlab.show()
        ...
        # print(key)
        # if key == 83:
        #     cv2.imwrite("avod_testresult_image/"+str(i)+".png", image_cnr)
    ...
    # img_boexs_gt = load_kittigt_annotation(image_index[i])["boxes"]
    # image_name = image_path_from_index(image_index[i])
    # image = cv2.imread(image_name)
    # for i in range(img_boexs_gt.shape[0]):
    #     # if gt_classes[i] != 2:
    #     #     continue
    ...
    #         continue
    #
    #     cv2.rectangle(image, (int(box_img[i,0]), int(box_img[i,1])), (int(box_img[i,2]), int(box_img[i,3])), (255, 255, 255), thickness=1)
